...
import { ROOM_TYPE_OPTIONS, type RoomTypeOption } from "./room-type-options";

export const PROPERTY_TYPE_OPTIONS: ReadonlyArray<{
  value: PropertyType;
  label: string;
}> = ROOM_TYPE_OPTIONS.map((option: RoomTypeOption) => ({
  value: option.value,
  label: option.en,
}));
      ...
      >
        ...
        {!institution || !selectionValid ? (
          <p className="search-filter-help">
            Choose an institution from the search results before updating
            filters.
          </p>
        ) : null}
