...
@Injectable()
export class ReportRateLimiter implements OnModuleDestroy {
  ...
  async record(
    studentId: string,
    report: { id: string; listingId: string; createdAt: Date },
  ): Promise<void> {
    const client = await this.getClient();
    if (!client) return;
    try {
      await client
        ...
        .zAdd(this.key(studentId), {
          score: report.createdAt.getTime(),
          value: `${report.id}:${report.listingId}`,
        })
        .expire(this.key(studentId), 3600)
        .exec();
    } catch {
      this.disable();
    } // Committed PostgreSQL reports stay authoritative.
  }

  private key(studentId: string) {
    return `findme:v1:report-limit:${studentId}`;
  }
